...
for j in tqdm(range(len(scale))):         # Sweep the different scales
    ...
    # Memorise the post ratio of extreme scales (for plotting purposes)
    if j==0:
    ...
    for p in range(n_jets):              # Check the flagged intervals
        ...
        if fun.not_detected(tt_jet, tt_jd, i_0, i_f):
...
if len(tt_jet)==0:                       # Case where no jet is detected
    ...
    # Final jet timetable
    final_tt_jet = Time(tt_jet[:, :2], format='jd').datetime  
                
    # Final associated log-posterior probability
    final_proba  = tt_jet[:, 2]           
